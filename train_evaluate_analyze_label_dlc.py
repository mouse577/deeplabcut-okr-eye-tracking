...
for pred_file in video_dir.glob(f"*{prediction_suffix}*.h5"):
    print(f"🔎 Found prediction: {pred_file.name}.h5")
    ...
    # video_file = pred_file.with_name(pred_file.name.replace(prediction_suffix, ".mp4"))
    base_name = pred_file.name.replace(prediction_suffix, "").split(".h5")[0]
    video_file = video_dir / f"{base_name}.mp4"

    if not video_file.exists():
        print(f"⚠️ Original video not found for {pred_file.name}, skipping.")
        continue
    ...
    print(f"✅ Saved labeled confidence video to: {output_file}")
